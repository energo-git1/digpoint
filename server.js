      ...
      if (!msgList.length) {
        ...
        for (const seq of msgList) {
          try {
            ...
            // kasimo.darbai@kaunas.lt — visada susiję su leidimu, keyword check praleisti
            const isKaunasSavivaldybe = fromAddr.toLowerCase().includes('kasimo.darbai@kaunas.lt');
            ...
            let quickBodyText = '';
            if (!subjectHasPermit && !isKaunasSavivaldybe) {
            ...
            const bodyHasPermit = isKaunasSavivaldybe || subjectHasPermit || PERMIT_BODY_KEYWORDS.test(quickBodyText);
            ...
            if (!pdfParts.length) {
              ...
              if (org === 'Kauno miesto savivaldybe' && fromAddr.toLowerCase().includes('kasimo.darbai@kaunas.lt')) {
                ...
                if (/išduotas leidimas/i.test(bodyText + ' ' + subject)) {
                  ...
                  // Kiti pranešimai (laukia apmokėjimo, pateiktas, atmestas ir pan.) — nieko nekeičiame
                  const statusMatch = bodyText.match(/Pra[sš]ymo\s+b[uū]sena[:\s]+([^\n<]{3,60})/i);
                  const detectedStatus = statusMatch ? statusMatch[1].trim() : '(nenustatyta)';
                  console.log(`[IMAP] Kauno sav.: informacinis laiškas (būsena: "${detectedStatus}") — statuso nekeičiame, praleidžiama.`);
                }
              } else {
...
function parseSavPermitText(text) {
  ...
  const dangosM = text.match(/Numatomos\s+ardyti\s+dangos[:\s]*([\s\S]